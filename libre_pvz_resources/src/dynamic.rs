...
pub trait AnyResource: Reflect + ErasedSerialize + Send + Sync + 'static {
    ...
    fn erased_deserialize(src: &mut dyn ErasedDeserializer) -> Result<DynamicResource, Error> where Self: Sized;
    ...
    fn erased_decode(reader: &mut dyn Reader) -> Result<DynamicResource, DecodeError> where Self: Sized;
}
...
pub struct ReflectAnyResource {
    ...
    get_boxed: fn(Box<dyn Reflect>) -> DynamicResource,
    erased_deserialize: fn(&mut dyn ErasedDeserializer) -> Result<DynamicResource, Error>,
    erased_decode: fn(&mut dyn Reader) -> Result<DynamicResource, DecodeError>,
}
...
impl ReflectAnyResource {
    ...
    /// Try to downcast a `Box<dyn Reflect>` type to `DynamicResource`.
    pub fn get_boxed(&self, val: Box<dyn Reflect>) -> DynamicResource { (self.get_boxed)(val) }
    /// Deserialize using [`serde`] into a trait object.
    pub fn erased_deserialize(&self, src: &mut dyn ErasedDeserializer) -> Result<DynamicResource, Error> {
        (self.erased_deserialize)(src)
    }
    /// Deserialize using [`bincode`] into a trait object.
    pub fn erased_decode(&self, reader: &mut dyn Reader) -> Result<DynamicResource, DecodeError> {
...
pub type DynamicResource = Box<dyn AnyResource>;

struct Wrapper<'a>(&'a dyn AnyResource);
...
impl Serialize for DynamicResource {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        let name = g.get(&self.type_id()).map(Box::as_ref)
            .ok_or_else(|| S::Error::custom(format_args!(
                "type '{}' does not support dynamic serialization", self.type_name())))?;
        let mut map = serializer.serialize_map(Some(2))?;
        map.serialize_entry(name, &Wrapper(self.as_ref()))?;
        map.end()
    }
}

impl<'de> DeserializeSeed<'de> for ReflectAnyResource {
    type Value = DynamicResource;
...
impl Encode for DynamicResource {
    fn encode<E: Encoder>(&self, encoder: &mut E) -> Result<(), EncodeError> {
        ...
        let name = g.get(&self.type_id()).map(Box::as_ref)
            .ok_or_else(|| EncodeError::OtherString(format!(
                "type '{}' does not support dynamic serialization", self.type_name())))?;
        name.encode(encoder)?;
        self.erased_encode(encoder.writer())
    }
}

impl Decode for DynamicResource {
